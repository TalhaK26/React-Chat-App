import React, { useState, useEffect, useRef } from 'react';
import './App.css';
import logo from './assets/logo.png';
import avatarImg from './assets/avatar.jpg';

var sendImg     = 'data:image/svg+xml;base64,';
var openWidget  = 'data:image/svg+xml;base64,';
var closeWidget = 'data:image/svg+xml;base64,';

function App() {
  const [message, setMessage] = useState('');
  const [avatar, setAvatar] = useState(avatarImg);
  const [messageList, setMessageList] = useState([]);
  const [isOpen, setIsOpen] = useState(false);
  const messageContainerRef = useRef(null);
  const [dimensions, setDimensions] = React.useState({ 
    height: window.innerHeight,
    width: window.innerWidth
  });
  const onMessageChange = (e) => setMessage(e.target.value); 
  const handleOpenChatBox = (e) => setIsOpen(!isOpen); 
  
  useEffect(() => {
    if(messageContainerRef.current) messageContainerRef.current.scrollTop = messageContainerRef.current.scrollHeight;
  }, [messageList])
  
  useEffect(() => {
    function handleResize() {
      setDimensions({
        height: window.innerHeight,
        width: window.innerWidth
      })
    }
    window.addEventListener('resize', handleResize);

    return _ => {
      window.removeEventListener('resize', handleResize);
    }
  })

  const hadnleSendMessage = (e) => {
    e.preventDefault();
    let _message = message.replace(/^\s+/, '').replace(/\s+$/, '');
    if(_message !== '') {
      let _messageList = [...messageList];
      _messageList.push({message: message});
      setMessageList(_messageList);
      setMessage('');
    }
  };

  return (
    <div>
      <div className={`rcw-widget-container ${isOpen ? 'rcw-opened' : ''}`}>
        { isOpen &&
          <div className="rcw-conversation-container">
            <div className="rcw-header">
              <button className="rcw-close-button"
                onClick={handleOpenChatBox}
              >
                <img src={closeWidget} className="rcw-close" alt="close" />
              </button>
              <h4 className="rcw-title"><img src={logo} className="avatar" alt="profile" /></h4>
              <span>Chat System for Engyj</span>
            </div>
            <div id="messages" ref={messageContainerRef} className="rcw-messages-container">
            { messageList.map((msg, index) => {
                if(index % 2 === 0) {
                  return <div key={index} className="rcw-message">
                    <div className="rcw-client">
                      <div className="rcw-message-text">
                        <p>{msg.message} 😌</p>
                      </div>
                    </div>
                  </div>
                }
                return <div className="rcw-message">
                  <img src={avatar} className="rcw-avatar" alt="profile" />
                  <div className="rcw-response">
                    <div className="rcw-message-text">
                        <p>{msg.message} 😘</p>
                    </div>
                  </div>
                </div>
              })
            }
            </div>
            <form className="rcw-sender" onSubmit={hadnleSendMessage}>
              <input type="text" className="rcw-new-message" name="message" placeholder="Type a message..." autoComplete="off" 
                value={message}
                onChange={onMessageChange}
              />
                <button type="submit" className="rcw-send" disabled={message===''}>
                  <img src={sendImg} className="rcw-send-icon" alt="send" />
                </button>
            </form>
          </div>
        }
        {/* <button type="button" className="rcw-launcher rcw-hide-sm" */}
        <button type="button" className="rcw-launcher"
          onClick={handleOpenChatBox}
          style={{display: isOpen && dimensions.width<800 ? 'none' : 'block'}}
        >
          <img src={isOpen ? closeWidget : openWidget} className="rcw-close-launcher" alt=""/>
        </button>
      </div>
    </div>
  );
}

export default App;
